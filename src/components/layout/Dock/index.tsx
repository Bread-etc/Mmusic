    ...
    <Drawer>
      <div className="app-region-no-drag absolute bottom-4 left-1/2 -translate-x-1/2 w-[95%] max-w-4xl h-20 px-4 bg-slate-50/60 dark:bg-slate-900/60 backdrop-blur-lg rounded-2xl shadow-lg border border-slate-200/80 dark:border-slate-700/80 flex items-center justify-between">
        ...
        <div className="flex-grow flex flex-col items-center justify-center mx-4">
          <div className="flex items-center gap-6">
            <Button onClick={(e) => { e.stopPropagation(); playPrev(); }} size="icon" variant="ghost" className="p-2 theme-text">
              <SkipBack size={22} />
            </Button>
            <Button onClick={(e) => { e.stopPropagation(); togglePlay(); }} size="icon" className="p-3 w-12 h-12 bg-sky-500 text-white shadow-md hover:scale-105 transition-transform">
              {isPlaying ? <Pause size={28} /> : <Play size={28} className="ml-1" />}
            </Button>
            <Button onClick={(e) => { e.stopPropagation(); playNext(); }} size="icon" variant="ghost" className="p-2 theme-text">
              <SkipForward size={22} />
            </Button>
          </div>
          <div className="w-full flex items-center gap-2 mt-1">
            <span className="text-xs theme-text w-10 text-center">{formatTime(currentTime)}</span>
            <Slider value={[currentTime]} max={duration} step={1} onValueChange={handleProgressChange} className="flex-grow" />
        ...
        <div className="w-64 flex items-center justify-end gap-4">
          <Button onClick={(e) => { e.stopPropagation(); toggleLike(); }} size="icon" variant="ghost" className={`p-1 ${songIsLiked ? "text-red-500" : "theme-text"}`}>
            <Heart size={20} className={`${songIsLiked ? "fill-current" : ""}`} />
          </Button>

          {/* 播放模式切换按钮 */}
          <Button onClick={(e) => { e.stopPropagation(); cyclePlaybackMode(); }} size="icon" variant="ghost" className="p-1 theme-text">
          ...
          >
            <Button onClick={(e) => { e.stopPropagation(); toggleMute(); }} variant="ghost" size="icon" className="theme-text">
      ...
      <DrawerContent className="h-full bg-transparent border-none outline-none">
        <div className="absolute inset-0 w-full h-full -z-20">
          <img src={currentSong.cover} className="w-full h-full object-cover blur-3xl scale-110" alt="background" />
          <div className="absolute inset-0 w-full h-full bg-black/50"></div>
        </div>
        <div className="w-full h-full flex flex-col items-center justify-center p-8 text-white">
          <DrawerClose asChild className="absolute top-6 left-6 app-region-no-drag">
            <Button variant="ghost" size="icon"><ChevronDown size={24} /></Button>
          </DrawerClose>
          <img src={currentSong.cover} alt={currentSong.title} className="w-80 h-80 rounded-lg shadow-2xl object-cover" />
